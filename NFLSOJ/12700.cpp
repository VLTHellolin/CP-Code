...
int n, base[N], a[N], temp[N];
vector<pair<int, int>> ans;
std::mt19937_64 mt(4731636727ll ^ std::chrono::steady_clock::now().time_since_epoch().count());
std::uniform_int_distribution<long long> dist;
void perform(int l, int r) {
    std::copy(&a[l], &a[r + 1], &temp[l]);
    ans.eb(l, r);
    int tmp = l;
...
void main() {
    read(n);
    // read_arr(&base[1], &base[n+1]);
    for (int i = 1; i <= n; ++i)
        base[read()] = i;
    do {
        ans.clear();
        std::copy(&base[1], &base[n + 1], &a[1]);
        for (int i = 1; i <= 100; ++i) {
            int l = dist(mt) % n + 1, r = dist(mt) % n + 1;
            if (l == r) continue;
            perform(min(l, r), max(l, r));
        }
        for (int i = n, j; i; --i) {
            if (a[i] == i) continue;
            j = i;
            ...
            if (i != j) {
                perform(min(i, j * 2 - i + 1), max(i, j * 2 - i + 1));
            }
        }
    } while (ans.size() > 6000);
    writeln(ans.size());
