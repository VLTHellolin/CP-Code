...
// clang-format on
namespace hellolin {
constexpr int N = 2e7 + 11;
constexpr ll Mod = 1e9 + 7;
int n;
ll alpha, beta;
modint<ll, Mod> one = 1, a, b, c;
modint<ll, Mod> p[N];
void main() {
#ifndef LOCAL
...
#endif
    read(n, alpha, beta);
    a = (1 - alpha + Mod) % Mod * beta % Mod;
    b = (((1 - alpha - beta) % Mod + 2 * alpha * beta % Mod) % Mod + Mod) % Mod;
    c = (1 - beta + Mod) % Mod * alpha % Mod;
    c = pow((ll) c, Mod - 2, Mod); // special
    p[1] = 1;
    p[2] = (one - b) * c;
    for (int i = 3; i <= n * 2; ++i)
        p[i] = ((one - b) * p[i - 1] - a * p[i - 2]) * c;
    writeln("{}", 1ll * pow((ll) p[n * 2], Mod - 2, Mod) % Mod * (ll) p[n] % Mod);
}
} // namespace hellolin