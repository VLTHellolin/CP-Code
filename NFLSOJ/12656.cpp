...
// clang-format on
namespace hellolin {
constexpr int N = 1e7 + 2, K = 1e6 + 2;
constexpr ll Mod = 998244353;
int n, k, d;
modint<ll, Mod> fac[N], ifac[N], ans;
int x[K];
modint<ll, Mod> c(int m, int n) {
    if (n < m) return 0;
    if (n < 0 || m < 0) return 0;
...
void main() {
    ...
    fac[0] = ifac[0] = 1;
    for (int i = 1; i <= n; ++i)
        ifac[i] = (fac[i] = fac[i - 1] * (modint<ll, Mod>) i).inverse();
    for (int q = read(); q--;) {
        ans = 0;
        read(k);
        read_arr(&x[1], &x[k + 1]);
