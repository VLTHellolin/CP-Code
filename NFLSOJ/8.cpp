...
ll pre[N * 2];
ll ans;
ll get_pre(ll l, ll r) {
    return pre[r] - pre[l - 1];
}
void main() {
    read(n), read_arr(&a[1], &a[n + 1]);
    ...
    for (int i = 1; i <= n; ++i) {
        auto chk = [&](ll p) {
            ll now = i + p - 1, cur = get_pre(i, i + p - 1);
            ll l = 0, r = n;
            while (r > l + 1) {
                ll mid = (l + r) >> 1;
                ll p1 = get_pre(now + 1, now + mid), p2 = get_pre(now + mid + 1, i + n - 1);
                if (p1 < cur) l = mid;
                if (p2 < cur) r = mid;
                if (p1 < cur && p2 < cur) return false;
                if (p1 >= cur && p2 >= cur) return true;
            }
            return false;
        };
        int l = 0, r = n;
