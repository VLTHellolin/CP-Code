...
int p[W];
void solve() {
    auto dfs = [&](auto fun, ll w, ll rs, bool qd, bool eq) -> ll {
        ll &ndp = dp[rs][w];
        if(!qd && !eq && ~ndp) return ndp;
        if(rs > 3) return 0;
        if(w < 1) return 1;
        ll ans = 0, qwq = eq ? p[w] : 9;
        rep(int, i, 0, qwq) {
            ans += fun(fun, w - 1, (i > 0) + rs, !i && qd, eq && i == qwq);
        }
        return qd || eq ? ans : ndp = ans;
    };
    rep(int, i, 0, 4) rep(int, j, 0, 19) dp[i][j] = -1;
    auto get_ans = [&](ll x) {
        tp = 0;
        do {
            p[++tp] = x % 10;
        ...
        return dfs(dfs, tp, 0, 1, 1);
    };
    for (std::cin>>T; T--; ) {
        std::cin >> l >> r;
        std::cout << get_ans(r) - get_ans(l-1) << nl;
    }
}
// * CORE CODE END * //
