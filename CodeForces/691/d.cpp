...
#include<vector>
namespace hellolin{template<class T>class dsu{private:int n;std::vector<T>pa,sz;public:dsu(){}explicit dsu(int _n):n(_n),pa(n+1),sz(n+1,0){for(int i=1;i<=n;++i)pa[i]=i;}void resize(int _n){n=_n;pa=std::vector<int>(n+1);sz=std::vector<int>(n+1,0);for(int i=1;i<=n;++i)pa[i]=i;}int find(int x){return pa[x]==x?x:pa[x]=find(pa[x]);}void merge(int x,int y){x=find(x),y=find(y);if(x==y)return;if(sz[x]<sz[y])std::swap(x,y);sz[x]+=sz[y];pa[y]=x;}int size(int x){return sz[find(x)];}bool same(int x,int y){return find(x)==find(y);}};}
#endif
// NOLINTEND
// clang-format on
namespace hellolin {
...
void main() {
    ...
    for (int i = 1; i <= n; ++i) {
        pq[d.find(i)].push(a[i]);
    }
    for (int i = 1; i <= n; ++i) {
        auto &x = pq[d.find(i)];
        write(x.top(), sorl(i, n));
        x.pop();
    }
}
} // namespace hellolin