...
void main() {
    for (int T = read(); T--;) {
        ...
        auto [a, b] = std::minmax_element(&p[1], &p[n + 1]);
        ans = *b - *a;
        auto c = *max_element(&l[1], &l[n + 1]);
        auto d = *min_element(&r[1], &r[n + 1]);
        for (int i = 1; i <= n; ++i)
            // chmax(ans, max(c - l[i], r[i] - d));
            chmax(ans, min(max(c - l[i], r[i] - d), p[i]));
        writeln("{}", ans << 1);
    }
}
} // namespace hellolin