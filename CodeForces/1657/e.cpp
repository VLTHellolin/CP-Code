...
int n, k;
modint<ll, Mod> fac[N], ifac[N], now, f[N][N];
modint<ll, Mod> c(int n, int m) {
...
void main() {
    ...
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j < k; ++j) {
            for (int l = i; l <= n; ++l) {
                now = c(n - i, l - i) * (modint<ll, Mod>) pow((ll) k - j, (i * (l - i) + (l - i - 1) * (l - i) / 2), Mod);
                // f[j][k+1] = f[i][j] * now;
                f[l][j + 1] += f[i][j] * now;
                // cerr << format("%lld %lld\n", f[i][j].x, now.x);
            }
