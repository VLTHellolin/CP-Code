...
ll dp[10][2];
std::vector<int> d;
// dp[i][0/1]: length=i, if the last number is 6
ll dfs(int now, bool is6, bool rea) {
    if(!now) return 1;
    ll &ndp = dp[now][is6];
    if(!rea && ~ndp) return ndp;
    ll ans = 0;
    rep(int, i, 0, rea ? d[now] : 9) {
        if(i != 4 && (!is6 || i != 2))
            ans += dfs(now - 1, i == 6, rea && i == d[now]);
    }
    return rea ? ans : ndp = ans;
}
ll get_ans(ll x) {
    d.clear();
    d.pb(0); // pos 0
...
void solve() {
    ...
    while(std::cin >> a >> b && a && b) {
        std::cout << get_ans(b) - get_ans(a - 1) << nl;
    }
}
// * CORE CODE END * //
