...
// * CORE CODE BEGIN * //
constexpr static int N = 11, Z = 4000, K = 114;
int n, k;
ll tb[Z], tbp, st[Z];
ll dp[N][Z][K], ans;
void solve() {
    ...
    auto cons = [&](auto fun, int now, int cnt, int den) -> void {
        if(den >= n) {
            st[++tbp] = cnt;
            tb[tbp] = now;
        } else {
            fun(fun, now, cnt, den + 1);
            // fun(fun, now + (1 << (den - 1)) , cnt + 1, den + 1);
            fun(fun, now + (1 << (den)) , cnt + 1, den + 2);
        }
    };
    std::cin >> n >> k;
    cons(cons, 0, 0, 0);
