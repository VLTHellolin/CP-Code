...
namespace hellolin {
constexpr static int N = 3005;
int n, k, sl, et, tmp;
int s, t;
graph::min_cost_flow<int, ll> mcf(N);
int x[N], y[N], eid[N];
ll res;
void solve() {
    std::cin >> n >> k >> sl >> et;
    sl = k - sl;
    s = 0, t = n + 1;
    mcf.add_edge(s, k, sl, 0);
    tmp = 0;
    rep(int, i, 1, n) {
        std::cin >> x[i];
    }
    res = std::accumulate(x+1, x+1+n, 0ll);
    rep(int, i, 1, n) {
        std::cin >> y[i];
        y[i] = x[i] - y[i];
        mcf.add_edge(i, i+1, sl - et, 0);
        int tar = i + k;
        if(tar > n) tar = t;
        mcf.add_edge(std::max(k, i), tar, 1,y[i]);
        ++tmp;
        eid[i] = ++tmp;
    }
    std::cout << res - mcf.get_flow_cost(s, t).se << nl;
    rep(int, i, 1, n) {
        auto e = mcf.get_edge(eid[i]);
        std::cout << (e.w - e.flow ? 'S' : 'E');
    }
    std::cout << nl;
