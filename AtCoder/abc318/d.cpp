// Problem: D - General Weighted Max Matching
...
int n;
ll d[N][N], ans;
class info { public: bool a[N]; };
void dfs(int now, info qwq, ll sum) {
    if(now > n) {
        chmax(ans, sum);
        return;
    }
    if(qwq.a[now]) dfs(now+1, qwq, sum);
    else
        rep(int, i, now, n)
            if(!qwq.a[i]) {
                qwq.a[i] = 1;
                dfs(now+1, qwq, sum + d[now][i]);
                qwq.a[i] = 0;
            }
}
void solve() {
