...
void solve() {
    ...
    std::sort(f+1, f+1+n, std::greater<ll>());
    rep(int, i, 1, n) pr[i] = pr[i-1] + f[i];
    rep(int, i, n+1, n+d+1) pr[i] = pr[n];
    for(int i=1; i<=n; i+=d) {
        ans += std::min(pr[i + d - 1] - pr[i - 1], p);
    }
    std::cout << ans << nl;
