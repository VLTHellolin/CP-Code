...
void main() {
  ...
  //     break;
  //   }
  // if (flag) return cout << *min_element(&c[1], &c[n + 1]) << '\n', void();
  for (int i = 1; i <= n; ++i) {
    bool ch = s[i] == '1';
    pre[i][ch] = pre[i - 1][!ch];
    pre[i][!ch] = pre[i - 1][ch] + c[i];
  }
  for (int i = n; i >= 1; --i) {
    bool ch = s[i] == '1';
    suf[i][ch] = suf[i + 1][!ch];
    suf[i][!ch] = suf[i + 1][ch] + c[i];
  }
  ans = 1e18;
  for (int i = 1; i < n; ++i)
    chmin(ans, min(pre[i][0] + suf[i + 1][0], pre[i][1] + suf[i + 1][1]));
  cout << ans << '\n';
}
} // namespace hellolin