// Hellolin's Code Template (with Main function) v4.0.1
// *INDENT-OFF*
// NOLINTBEGIN
// clang-format off
#define HLCTv4 1
...
void main() {
    read(n, k);
    auto fu = [&](const ll x) { return x < 2 ? 0 : (x - 1) * (x - 2) / 2; };
    for (ll i = 3; i <= 3 * n; ++i) {
        now = fu(i) - 3 * fu(i - n) + 3 * fu(i - n * 2) - fu(i - n * 3);
        if (k > now) {
            k -= now;
            continue;
        }
        for (ll x = 1; x <= n; ++x) {
            int l = max(1ll, i - x - n), r = min(n, i - x - 1), len = r - l + 1;
            if (l <= r) {
                if (k > len) {
                    k -= len;
                    continue;
                }
                writeln("{} {} {}", x, l + k - 1, i - l - x - k + 1);
                return;
            }
        }
    }
