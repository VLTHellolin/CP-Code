...
void main() {
    read(n);
    atcoder::segtree<ll, [&](const ll &x, const ll &y) { return min(x, y); }, [&]() { return Inf; }> st(n + 1);
    for (int i = 1; i <= n; ++i)
        p[read()] = i;
    for (int i = 1; i <= n; ++i)
        read(a[i], b[i], c[i]);
    for (int i = 1; i <= n; ++i) {
        preA[i] = preA[i - 1] + a[i];
        preB[i] = preB[i - 1] + min(a[i], b[i]);
        preC[i] = preC[i - 1] + min(a[i], c[i]);
    }
    for (int i = 1; i <= n; ++i) {
        ll now = min(preA[i - 1] + st.prod(0, p[i]), preB[i - 1]);
        st.set(p[i], now - preA[i]);
        chmin(ans, now + (preC[n] - preC[i]));
    }
    writeln(ans);
}
} // namespace hellolin