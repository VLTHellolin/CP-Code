...
// clang-format on
namespace hellolin {
constexpr int N = 1e5 + 11, Inf = 9e8;
int n, m;
int f[N], g[N];
void main() {
    read(n, m);
    ...
    for (int i = 1; i <= n; ++i)
        for (int j = max(1, i - m); j < i; ++j)
            if (s[j - 1][i - j - 1] == '1') chmin(f[i], f[j] + 1);
    for (int i = n; i >= 1; --i)
        for (int j = min(n, i + m); j > i; --j)
            if (s[i - 1][j - i - 1] == '1') chmin(g[i], g[j] + 1);
    for (int i = 2; i < n; ++i) {
        int ans = Inf;
        for (int j1 = max(1, i - m); j1 < i; ++j1)
            for (int j2 = min(n, i + m); j2 > i; --j2)
                if (j2 - j1 <= m && s[j1 - 1][j2 - j1 - 1] == '1')
                    chmin(ans, f[j1] + g[j2] + 1);
        write(ans == Inf ? -1 : ans, ' ');
    }
    writeln();
}
