...
void solve() {
    for(std::cin>>q; q--; ) {
        ...
        if(op == 1) {
            ...
            if(it != now.end()) { // exists
                ans.insert(*it ^ x);
            }
            if(it != now.begin()) { // back 1
                auto itt = it;
                --itt;
                ans.insert(*itt ^ x);
            }
            if(it != now.begin() && it != now.end()) { // front & back
                auto itt = it--;
                ans.erase(ans.find(*it ^ *itt));
            }
            now.insert(x);
        } else if(op == 2) {
            std::cin >> x;
