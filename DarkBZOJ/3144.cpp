...
namespace hellolin {
constexpr static int N = 50, INF = 5e8;
int a;
int s, t;
int p, q, r, d;
int dx[5] = {-1, 0, 0, 1, -1};
int dy[5] = {-1, 1, -1, 0, 0};
graph::max_flow<int> mf(100000);
...
void solve() {
    ...
    rep(int, i, 1, r)
        rep(int, j, 1, p)
            rep(int, k, 1, q) {
                std::cin >> a;
                mf.add_edge(calc(j, k, i-1), calc(j, k, i), a);
            }
    rep(int, i, 1, p)
        rep(int, j, 1, q) {
            mf.add_edge(s, calc(i, j, 0), INF);
            mf.add_edge(calc(i, j, r), t, INF);
        }
    rep(int, i, d, r)
        rep(int, x, 1, p)
            rep(int, y, 1, q)
                rep(int, j, 1, 4) {
                    ...
                    if(kx >= 1 && kx <= p && ky >= 1 && ky <= q)
                        mf.add_edge(calc(x, y, i), calc(kx, ky, i - d), INF);
                }
    std::cout << mf.get_flow(s, t) << nl;
}
} // namespace hellolin