...
namespace hellolin {
constexpr static int N = 1e4 + 114;
int n, m, s, t, x;
graph::min_cost_flow<int, int> mcf(N);
void solve() {
    std::cin >> m >> n;
    s = 0, t = (n+1) * (m+1) + 1;
    rep(int, i, 1, n) mcf.add_edge(s, i, 1, 0);
    rep(int, i, 1, m)
        rep(int, j, 1, n)
            mcf.add_edge(i * n + j, t, 1, 0);
    rep(int, i, 1, n)
        rep(int, j, 1, m) {
            std::cin >> x;
            rep(int, k, 1, n)
                mcf.add_edge(i, j * n + k, 1, k * x);
        }
    std::cout << std::fixed << std::setprecision(2) << mcf.get_flow_cost(s, t).se * 1.0 / n << nl;
}
} // namespace hellolin