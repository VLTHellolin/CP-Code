...
namespace hellolin {
constexpr static int N = 100, INF = 8e8;
int n, m, r, s, t;
int u[N], d[N];
graph::min_cost_flow<int, int> mcf(N);
void solve() {
    std::cin >> n >> m >> r;
    rep(int, i, 1, n) std::cin >> u[i];
    rep(int, i, 1, n) std::cin >> d[i];
    s = 0, t = n + 1;
    rep(int, i, 1, n) {
        if(i != n) mcf.add_edge(i, i+1, r, m);
        mcf.add_edge(i, t, u[i], 0);
        mcf.add_edge(s, i, INF, d[i]);
    }
