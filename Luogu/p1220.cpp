...
void main() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int l = 1; l <= n; ++l) {
            int r = l + i - 1;
            if (r > n) break;
            int cur = (pw[n] + pw[l - 1] - pw[r]);
            chmin(f[l - 1][r][0], min(f[l][r][0] + cur * (d[l] - d[l - 1]), f[l][r][1] + cur * (d[r] - d[l - 1])));
            chmin(f[l][r + 1][1], min(f[l][r][1] + cur * (d[r + 1] - d[r]), f[l][r][0] + cur * (d[r + 1] - d[l])));
        }
    }
