...
namespace hellolin {
constexpr static int N = 1e4 + 514;
int k, n, m, s, t, p, x;
int ty[N];
...
void solve() {
    std::cin >> k >> n;
    rep(int, i, 1, k) std::cin >> ty[i];
    m = std::accumulate(ty+1, ty+1+k, 0);
    s = 0, t = k + n + 1;
    rep(int, i, 1, k) {
        mf.add_edge(s, i, ty[i]);
        mf.add_edge(i, s, 0);
    }
    rep(int, i, 1, n) {
        std::cin >> p;
    ...
    if(mf.get_flow(s, t) != m) {
        std::cout << "No Solution!\n";
    } else {
        auto vis = mf.min_cut(s);
        auto ed = mf.get_edges();
        rep(int, i, 1, k) {
            std::cout << i << ": ";
            for(auto j : ed) {
                if(j.fr == i) {
                    if(j.to != s && !vis[j.to] && !(j.w - j.flow))
                        std::cout << j.to - k << ' ';
                }
            }
            std::cout << nl;
