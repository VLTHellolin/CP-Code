...
namespace hellolin {
constexpr static int N = 1e3 + 11, INF = 8e8;
int n, s, t, a[N], avg;
...
void solve() {
    ...
    avg = std::accumulate(a+1, a+1+n, 0) / n;
    s = 0, t = n << 1 | 1;
    mcf.add_edge(1, n, INF, 1);
    mcf.add_edge(n, 1, INF, 1);
    mcf.add_edge(1, n+n, INF, 1);
    mcf.add_edge(n, n+1, INF, 1);
    rep(int, i, 1, n) {
        if(a[i] >= avg)
            mcf.add_edge(s, i, a[i] - avg, 0);
        else
            mcf.add_edge(i+n, t, avg - a[i], 0);
        
        if(i > 1) {
            mcf.add_edge(i, i-1, INF, 1);
            mcf.add_edge(i, i+n-1, INF, 1);
        }
        if(i < n) {
            mcf.add_edge(i, i+1, INF, 1);
            mcf.add_edge(i, i+n+1, INF, 1);
