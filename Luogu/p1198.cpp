...
ll tr[N << 2];
void pull(int x = 1) {
    tr[x] = max(tr[x << 1], tr[x << 1 | 1]) % d;
}
void modify(int p, int l, int r, int w, int x = 1) {
    if (l == r) return tr[x] = w, void();
    int mid = (l + r) >> 1;
    if (mid >= p)
        modify(p, l, mid, w, x << 1);
    else
        modify(p, mid + 1, r, w, x << 1 | 1);
    pull(x);
}
ll query(int l, int r, int lx, int rx, int x = 1) {
    if (l <= lx && rx <= r) return tr[x];
    int mid = (lx + rx) >> 1;
    ll ans = 0;
    if (mid >= l) chmax(ans, query(l, r, lx, mid, x << 1));
...
void main() {
    ...
    for (int i = 1; i <= q; ++i) {
        ...
        read(ty, x);
        if (ty == 'Q') {
            writeln(lst = segment_tree::query(n - x + 1, n, 1, q));
        } else {
            segment_tree::modify(++n, 1, q, (x + lst) % d);
        }
    }
}
} // namespace hellolin