...
constexpr static int N = 1e3 + 114;
int T, n, m, s, t, a, b;
int x[N], l[N], r[N], pfx[N];
ld p;
...
void solve() {
    for(std::cin >> T; T--; [&](){
        ...
        rep(int, i, s+1, t) {
            mcf.add_edge(i-1, i, a, 0);
        }
        rep(int, i, 1, n) {
            mcf.add_edge(l[i], i+m+1, 1, 0);
            mcf.add_edge(i+m+1, r[i], 1, x[i]);
            // if(x[i] > p * x[i])
            mcf.add_edge(i+m+1, l[i]+1, 1, x[i] - floor(p * x[i] + 1e-6));
        }
        std::cout << std::accumulate(x+1, x+1+n, 0) - mcf.get_flow_cost(s, t).se << nl;
