...
namespace hellolin {
constexpr static int N = 1e6 + 514, INF = 8e8;
int n, k, a, b, c, s, t, cur;
bool x;
...
void solve() {
    ...
    auto add = [&](int i, int j, int c) {
        cur = calc(i, j, c);
        if(i > 1) mcf.add_edge(cur, calc(i-1, j, c+1), 1, b);
        if(i < n) mcf.add_edge(cur, calc(i+1, j, c+1), 1, 0);
        if(j > 1) mcf.add_edge(cur, calc(i, j-1, c+1), 1, b);
        if(j < n) mcf.add_edge(cur, calc(i, j+1, c+1), 1, 0);
    };
    std::cin >> n >> k >> a >> b >> c;
    s = 0, t = (k+1) * n * n + 1;
    mcf.add_edge(s, calc(1, 1, 0), 1, 0);
    rep(int, i, 1, k) mcf.add_edge(calc(n, n, i), t, 1, 0);
    rep(int, i, 1, n) {
        rep(int, j, 1, n) {
            std::cin >> x;
            if(!x) {
                mcf.add_edge(calc(i, j, k), calc(i, j, 0), 1, a + c);
                rep(int, c, 0, k - 1)
                    add(i, j, c);
            } else {
                rep(int, c, 1, k)
                    mcf.add_edge(calc(i, j, c), calc(i, j, 0), 1, a);
                add(i, j, 0);
            }
        }
    }
