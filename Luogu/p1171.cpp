...
// clang-format on
namespace hellolin {
constexpr int N = 30, Rn = 1 << 20, Inf = 7e8;
int n;
int f[Rn][N], a[N][N], ans;
void main() {
    read(n);
    ans = Inf;
    for (int i = 1; i <= n; ++i)
        read_arr(&a[i][1], &a[i][n + 1]);
    for (int i = 0; i < Rn; ++i)
        std::fill(&f[i][1], &f[i][n + 1], Inf);
    f[1][1] = 0;
    for (int i = 0; i < (1 << n); ++i) {
        for (int j = 1; j <= n; ++j) {
            if ((1 << (j - 1)) & i) continue;
            for (int k = 1; k <= n; ++k) {
                if ((1 << (k - 1)) & i)
                    chmin(f[i | (1 << (j - 1))][j], a[k][j] + f[i][k]);
            }
        }
    }
    for (int i = 1; i <= n; ++i)
        chmin(ans, a[i][1] + f[(1 << n) - 1][i]);
    writeln(ans);
}
} // namespace hellolin