...
namespace hellolin {
constexpr static int N = 3e3 + 11;
constexpr static ll INF = 2e14;
int n, r[N], s, t;
int p, fd, fc, sd, sc;
graph::min_cost_flow<ll, ll> mcf(N<<1);
void solve() {
    std::cin >> n;
    rep(int, i, 1, n) std::cin >> r[i];
    std::cin >> p >> fd >> fc >> sd >> sc;
    s = 0, t = n << 1 | 1;

    rep(int, i, 1, n) {
        mcf.add_edge(s, i, r[i], 0);
        mcf.add_edge(i+n, t, r[i], 0);

        if(i != n)
            // mcf.add_edge(i+n, i+n+1, INF, 0);
            mcf.add_edge(i, i+1, INF, 0);
        mcf.add_edge(s, i+n, INF, p);

        if(i + fd <= n)
            mcf.add_edge(i, i+n+fd, INF, fc);
        if(i + sd <= n)
            mcf.add_edge(i, i+n+sd, INF, sc);
    }
