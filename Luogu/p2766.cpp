...
namespace hellolin {
constexpr static int N = 1010, INF = 6e8;
int n, a[N], s, t;
int dp[N];
int ans1, ans2, ans3;
graph::max_flow<int> mf(N);
void solve() {
    std::cin >> n;
    ...
    rep(int, i, 1, n) {
        mf.add_edge(i, i+n, 1);
        if(dp[i] == 1) mf.add_edge(s, i, 1);
        else if(dp[i] == ans1) mf.add_edge(i+n, t, 1);
        rep(int, j, 0, i - 1)
            if(a[i] >= a[j])
                if(dp[j] + 1 == dp[i])
                    mf.add_edge(j+n, i, 1);
    }
    ans2 = mf.get_flow(s, t);

    mf.add_edge(s, 1, INF);
    if(dp[n] + 1 == dp[n+1]) mf.add_edge(n+n, t, INF);
    mf.add_edge(1, 1+n, INF);
    mf.add_edge(n, n+n, INF);
    ans3 = ans2 + mf.get_flow(s, t);

    std::cout << ans1 << nl << ans2 << nl << ans3 << nl;
}
} // namespace hellolin