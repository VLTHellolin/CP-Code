// Hellolin's Code Template (with Main function) v4.0.3
// *INDENT-OFF*
// NOLINTBEGIN
// clang-format off
#define HLCTv4 1
...
// clang-format on
namespace hellolin {
constexpr ll N = 1141, Inf = 1e15;
array<ll, 3> a[N];
ll ans, su, n, lc, f[N][N][2];
void main() {
    read(n, lc);
    a[n + 1][0] = lc;
    for (int i = 1; i <= n; ++i)
    ...
    for (int i = 1; i <= n; ++i) {
        for (int l = 1; l <= n; ++l) {
            int r = l + i;
            if (r > n) break;
            f[l][r][0] = min(f[l + 1][r][0] + (a[l + 1][0] - a[l][0]) * (a[n][2] + a[l][2] - a[r][2]), f[l + 1][r][1] + (a[r][0] - a[l][0]) * (a[n][2] + a[l][2] - a[r][2]));
            f[l][r][1] = min(f[l][r - 1][0] + (a[r][0] - a[l][0]) * (a[n][2] + a[l - 1][2] - a[r - 1][2]), f[l][r - 1][1] + (a[r][0] - a[r - 1][0]) * (a[n][2] + a[l - 1][2] - a[r - 1][2]));
        }
    }
    cout << std::fixed << std::setprecision(3) << (su - min(f[1][n][0], f[1][n][1])) / 1000.0l << newl;
}
} // namespace hellolin