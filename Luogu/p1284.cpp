...
bool chk() {
    sort(sl + 1, sl + 4);
    return sl[1] + sl[2] > sl[3];
}
int calc(int x, int y, int z) {
    llf t = (x + y + z) / 2.0l;
    return std::trunc(sqrt(t * (t - x) * (t - y) * (t - z)) * 100);
}
void main() {
    read(n);
    for (int i = 1; i <= n; ++i)
        read(a[i]);
    su = accumulate(a + 1, a + 1 + n, 0);
    res = -1;
    f[0][0] = true;
    for (int k = 1; k <= n; ++k) {
        for (int i = su / 2; ~i; --i) {
            for (int j = su / 2; ~j; --j) {
                (i >= a[k] && f[i - a[k]][j]) && (f[i][j] = true);
                (j >= a[k] && f[i][j - a[k]]) && (f[i][j] = true);
            }
        }
    }
    for (int i = su / 2; ~i; --i) {
        for (int j = su / 2; ~j; --j) {
            if (f[i][j]) {
                sl[1] = i, sl[2] = j, sl[3] = su - i - j;
                if (chk()) chmax(res, calc(sl[1], sl[2], sl[3]));
            }
        }
    }
    writeln("{}", res);
